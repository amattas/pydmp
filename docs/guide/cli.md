...
```bash
pydmp arm-away <AREA> [--bypass-faulted] [--force-arm]
pydmp arm-stay <AREA> [--bypass-faulted] [--force-arm]
pydmp disarm <AREA>
```
...
```bash
pydmp arm-areas "1,2,3" [--bypass-faulted] [--force-arm] [--instant/--no-instant]
pydmp disarm-areas "1,2,3"
```
...
### Zones
```bash
pydmp bypass-zone <ZONE>
pydmp restore-zone <ZONE>
```
Sends `!X` or `!Y` for a 3‑digit zone number (e.g., `005`).

### Outputs
```bash
pydmp output <OUTPUT> on|off|pulse|toggle
```
...
```bash
pydmp listen [--host 0.0.0.0] [--port 5001] [--duration 0]
```
Starts the S3 listener and prints parsed events. Use `Ctrl+C` to stop or `--duration` to exit after N seconds.
